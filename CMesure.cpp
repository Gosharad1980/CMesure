...
double CMesure::Fx(double _input, bool _inv)
{
	...
	double fx[2][nb_points] = {{ 99.999, 99.998, 99.99,  99.98,   99.9,   99.8,   99.0,  98.0,  96.0, 95.45, 95.0,  94.0,  92.0,  90.0,  88.0,  86.0,  84.0,  82.0,  80.0, 68.27,   60.0,   50.0, 38.29,   30.0, 0.0 }, //p
	...
	else if(_input <= fx[x][nb_points-1]) return fx[y][nb_points-1];
	else
	{
		// Recherche du cadran dans lequel on se situe
		for(i=1; i<nb_points; i++) if(_input >= fx[x][i]) break;
